...
DEBIAN_IMAGE="debian-12-generic-amd64-${CLOUD_RELEASE}.qcow2"
# Missing kernal support for USB
# DEBIAN_IMAGE="debian-12-genericcloud-amd64-${CLOUD_RELEASE}.qcow2"
CLOUD_INIT_FILE='debian-cloudinit.yaml'

VM_ID=10000
STORAGE_GB='32G'
VM_TEMPLATE="debian-bookworm-${STORAGE_GB}"
ZFS_POOL='local-cluster-zfs'
NETWORK_BRIDGE='vmbr0'
TEMPLATE_TAGS='debian-12,debian-bookworm,cloudinit'
...
echo "## Downloading $DEBIAN_IMAGE debian image. ##"
wget -q https://cdimage.debian.org/images/cloud/bookworm/20251006-2257/${DEBIAN_IMAGE}


# Resize image otherwise VMs will be created with no free space
echo "## Resizing $DEBIAN_IMAGE debian image to $STORAGE_GB. ##"
qemu-img resize $DEBIAN_IMAGE $STORAGE_GB
...
# Import the debian image to the VM and configure boot. Enable DHCP
echo "## Importing $DEBIAN_IMAGE image to $ZFS_POOL. Configuring storage and network. ##"
qm importdisk $VM_ID $DEBIAN_IMAGE $ZFS_POOL                      # Use the downloaded and resized image
